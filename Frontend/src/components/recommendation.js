...
const Recommendation = () => {
    ...
    const handleRecommendation = async () => {
        try {
            console.log("Sending Request with:", { skinType, category, brand });
            ...
            <div className="form-container">
                <div className="form-group">
                    ...
                    <select onChange={(e) => setSkinType(e.target.value)} value={skinType}>
                        ...
                        <option value="Combination,sensitive,Normal"> Combination,sensitive,Normal </option>
                    </select>
                </div>
  ...
  <h2 className='Rec-pro'>Recommended Products</h2>
  {products.length > 0 ? (
        ...
        <div className="card" key={index}>
          <div className="card-inner">
            <div className="card-front">
              <h3 className='product-hea'>{product.productName}</h3>
              
            </div>
            <div className="card-back">
            <p className='benefits'>Benefits: {product.benefits || 'No benefits listed'}</p>
             <p className='sentiment'>Sentiment: {product.sentiment || 'No sentiment available'}</p>
              
              <p className='price'>Price: {product.price ? `${product.price} PKR` : 'Price not available'}</p>
            </div>
          </div>
        </div>
      ))}
